      ...
      return ctx.db.connection().execute(async (conn) =>
        ...
        conn
          ...
          .select([
            ...
            'friends.phoneNumber',
            'totalFriendCount',
            userMutualFriendCount(conn, ctx.session.userId, input.friendUserId).as('mutualFriendCount'),
          ])
...
const userMutualFriendCount = (db: Database, userId: number, friendUserId: number) => {
  return db
    .selectFrom('friendships as my')
    .innerJoin('friendships as their', 'my.friendUserId', 'their.friendUserId')
    .select((eb) => [
      eb.fn.count('my.friendUserId').as('mutualFriendCount'),
    ])
    .where('my.status', '=', FriendshipStatusSchema.Values['accepted'])
    .where('my.userId', '=', userId)
    .where('their.userId', '=', friendUserId)
    .groupBy('my.userId')
}
